...
```
Using device: GPU
Model summary: online DuelingDQN 50.2K params, target DuelingDQN 50.2K params

關鍵里程碑（win_rate_100 最佳值）：
  Epoch   ~50  → win_rate_100 = 0.47
  Epoch  ~310  → win_rate_100 = 0.60
  Epoch  ~500  → win_rate_100 = 0.65
  Epoch  ~605  → win_rate_100 = 0.72
  Epoch  ~944  → win_rate_100 = 0.88
  Epoch ~1150  → win_rate_100 = 0.90
  Epoch ~1600  → win_rate_100 = 0.94
  Epoch ~1809  → win_rate_100 = 0.96
  Epoch ~2195  → win_rate_100 = 0.98
  Epoch ~2221  → win_rate_100 = 0.99
  Epoch ~3154  → win_rate_100 = 1.00  ← 首次達到 100%

Final win rate (500 eps): 95.8%
Best checkpoint saved: best-epoch=3154-win_rate_100=1.00.ckpt
train_loss (final): 0.110
```

**結果分析：**

Lightning DQN 在 random mode 達到 **95.8%** 最終勝率，且在 epoch ~3154 首次達到 100% win_rate（100 episode rolling average）。

這個優秀結果歸功於多個訓練技巧的組合：

| 技巧 | 貢獻 |
|------|------|
| **Dueling 架構** | 讓 V(s) 快速學到「靠近 Goal = 有價值」，無論 Goal 在哪裡 |
| **Double DQN** | 消除 Q 值過估，訓練更穩定 |
| **Gradient Clipping** | 防止早期大 TD error 造成梯度爆炸 |
| **StepLR 排程** | epoch 1000/2000/3000 時 lr 各乘 0.5，讓後期精細收斂 |
| **Huber Loss** | 對離群 transition 更穩健，減少震盪 |
| **ModelCheckpoint** | 自動儲存 win_rate 最高的 checkpoint，不怕後期震盪 |
...
| HW3-3 | Lightning DQN | random | **95.8%** | ~3154 |
| HW3-4 | Rainbow DQN | random | **67.7%**（訓練峰值） | 5000 |

> 以上皆為實際跑出的結果（硬體：RTX 5060 Laptop，CUDA 12.8，PyTorch 2.10.0）  
> HW3-4 最終 greedy 評估 23%；訓練中 ε-greedy 最高達 67.7%（詳見上方分析）  
> HW3-3 Lightning 在 epoch ~3154 首次達到 100% rolling win rate，最終 greedy eval 95.8%
